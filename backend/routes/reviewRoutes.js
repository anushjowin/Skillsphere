...
router.post("/", authMiddleware, async (req, res) => {
    try {
        ...
        const isParticipant = [String(targetGig.client), String(targetGig.freelancer)].includes(String(req.user.id))
            && [String(targetGig.client), String(targetGig.freelancer)].includes(String(reviewee));
        if (!isParticipant) {
            return res.status(403).json({ msg: "Only gig participants can review each other" });
        }
        ...
        if (existing) {
            return res.status(400).json({ msg: "You already reviewed this user for this gig" });
        }
